  ...
  private handleResize(gameSize: Phaser.Structs.Size) {
    ...
    if (this.wordContainer) {
        ...
        if (speakerBg) {
            ...
            speakerBg.setPosition(wordText.x + wordText.width/2 + 60 * scale, 80 * scale);
        }
        if (speakerIcon) {
    ...
    if (this.muteButton) {
        this.muteButton.setPosition(newWidth - padding, padding);
        const buttonBg = this.muteButton.list[0] as Phaser.GameObjects.Arc;
        buttonBg.setRadius(20 * scale);
        const buttonIcon = this.muteButton.list[1] as Phaser.GameObjects.Text;
        buttonIcon.setFontSize(24 * scale);
    }
  }
...
private displayWord() {
    ...
    // Create word text with highlighted first letter
    const word = this.currentWord.word.toUpperCase();
    const firstLetter = word.charAt(0);
    const restOfWord = word.slice(1);

    // Create container for the word and center it
    const wordTextContainer = this.add.container(0, 80);

    // Add first letter with highlight
    const firstLetterText = this.add.text(0, 0, firstLetter, {
        fontSize: '48px',
        fontFamily: 'Comic Sans MS',
        color: '#ff0000',  // Red color for first letter
        stroke: '#ffff00', // Yellow stroke for emphasis
        strokeThickness: 4
    }).setOrigin(0.5, 0.5);

    // Add rest of the word
    const restOfWordText = this.add.text(firstLetterText.width/2, 0, restOfWord, {
        fontSize: '48px',
        fontFamily: 'Comic Sans MS',
        color: '#000000',
        stroke: '#ffffff',
    ...
    // Add texts to word container
    wordTextContainer.add([firstLetterText, restOfWordText]);

    // Calculate total width for centering
    const totalWidth = firstLetterText.width + restOfWordText.width;
    
    // Center the word container
    wordTextContainer.setX(-totalWidth / 2);

    // Add speaker icon next to the word with more spacing
    const speakerBg = this.add.circle(wordTextContainer.x + totalWidth + 100, 80, 25, 0x4CAF50)
        .setInteractive({ useHandCursor: true });
    const speakerIcon = this.add.text(speakerBg.x, speakerBg.y, '🔊', {
        fontSize: '32px'
    }).setOrigin(0.5);
    ...
    // Add everything to the container
    this.wordContainer.add([overlay, bg, image, wordTextContainer, speakerBg, speakerIcon, closeButton, closeX]);
    ...
    // Add pulsing animation to first letter
    this.tweens.add({
        targets: firstLetterText,
        scale: 1.2,
        duration: 500,
        yoyo: true,
        repeat: -1,
  ...
  private createWordCard(word: string, highlightLetter: string, x: number, y: number): Phaser.GameObjects.Container {
    ...
    word.split('').forEach(char => {
      const isHighlighted = char.toLowerCase() === highlightLetter.toLowerCase();
  ...
  private createHighlightedWord(word: string, letter: string, x: number, y: number, container: Phaser.GameObjects.Container) {
    ...
    chars.forEach((char, i) => {
      const isFirstLetter = i === 0;  // Check if it's the first letter
      const charText = this.add.text(x + xOffset, y, char, {
        fontSize: '28px',
        fontFamily: 'Comic Sans MS',
        color: isFirstLetter ? '#ff0000' : '#4A4A4A',
        stroke: isFirstLetter ? '#ffff00' : null,
        strokeThickness: isFirstLetter ? 4 : 0
      });
  ...
  private createMuteButton() {
    ...
    this.muteButton = this.add.container(this.cameras.main.width - padding, padding);
    
    const buttonBg = this.add.circle(0, 0, 20 * scale, 0x4CAF50);
    const buttonIcon = this.add.text(0, 0, this.isMuted ? '🔇' : '🔊', {
        fontSize: `${24 * scale}px`
    }).setOrigin(0.5);
    
    this.muteButton.add([buttonBg, buttonIcon]);
    ...
    buttonBg.setInteractive({ useHandCursor: true })
        ...
        .on('pointerdown', () => {
            ...
            buttonIcon.setText(this.isMuted ? '🔇' : '🔊');
            if (this.bgMusic) {
                this.bgMusic.setMute(this.isMuted);
            }
            // Save mute state
